...
import { socialType, socialMediaType } from '../types'
...
const EditSocialProfile: React.FC<propsType> = ({
  ...
  return (
    <section
      className='w-[440px] bg-white dark:bg-[#303030] dark:text-white max-w-[95%] mx-auto p-[10px] rounded-sm fixed z-30 top-20 
  left-1/2 transform -translate-x-1/2 shadow-md border border-[#dad9d9ab]'
    >
      ...
      <form onSubmit={formSubmitHandler}>
        <div className='form-control relative w-[333px] max-w-full w-max-full h-[47px] mx-auto mt-[17px]'>
          {isReadOnly ? (
            <input
              type='text'
              name=''
      ...
      border-[#c2c2c2ab] text-[#3F3F3F] dark:text-white'
        ...
        {!isReadOnly && (
          <button
            className='w-[62px] block h-[32px] rounded-sm bg-main text-white bold ml-auto mt-2'
            type='submit'
          >
            Save
          </button>
